<template>
	<section>
		...
		<!--列表  显示学生信息-->
		<el-table :data="users"  @selection-change="selsChange"  highlight-current-row ref = "multipleTable" style="width: 100%;">
...
/*
			...
			getUsers() {
				...
				this.$axios.get('http://localhost:8083/Student/Page',{params:{indexpage:para.indexpage,everypage:para.everypage}})
				.then(function (response) {
			...
			getHAHA(data) {
				var second_this = this;
				this.$axios.post('http://111.231.104.12:8051/index/app/tehPage',qs.stringify(
				{
					data:data,
				}))
				.then(function (response) {
							...
							sec_this.$axios.put('http://localhost:8083/User', formData)
								.then(function (response) {
									...
									if(response.data['msg'] ==="add success")
									{
										...
										sec_this.getUsers();
										sec_this.editFormVisible = false;
										sec_this.$refs['editForm'].resetFields();  //重置页面
									}
									else{
										sec_this.editLoading = false;
										sec_this.$message({
