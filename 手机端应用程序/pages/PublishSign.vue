<template>
  <div>
    <div>
      <x-header :left-options="{backText: ''}" style="background-color: #3399ff;">签到</x-header>
    </div>
    <div class="content">
...
export default {
  ...
  methods: {
    ...
    showData: function () { // 直接显示教师本课程的历史发布签到
      ...
      axios.get('http://47.106.217.11:8083/Teacher/SignIn', {
        params: {
    ...
    handleSubmit: function (value) { // 发布签到
      ...
      axios.post('http://111.231.104.12:8051/index/app/sfqd', qs.stringify({
      ...
      this.userName = localStorage.getItem('username')
      this.courseId = localStorage.getItem('courseid')
      axios.post('http://47.106.217.11:8083/Teacher/SignIn', qs.stringify({
        courseid: this.courseId,
    ...
    handleInfo: function (id) { 
      ...
      axios.get('http://47.106.217.11:8083/Teacher/SignRecord', {
        params: {
          signid: parseInt(id)
        }
      }).then(res => {
